...
func GetTopOrder() ([]TradeOrder, error) {
	...
	sql := fmt.Sprintf("select top %d shelp,logistics_comp,logistics_order FROM trade_order(nolock)  where is_subscribe = 0 and send_date >= ?1 ORDER BY send_date ASC;", top_count)
	rows, err := DB.Queryx(sql, start_data)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
